...
var (
	...
	authUsersRowsExpectAutoSet   = strings.Join(stringx.Remove(authUsersFieldNames, "`user_id`", "`create_at`", "`create_time`", "`created_at`", "`update_at`", "`update_time`", "`updated_at`"), ",")
...
type (
	...
	AuthUsers struct {
		...
		Username sql.NullString `db:"username"`  // 用户登录名
		Password sql.NullString `db:"password"`  // 用户登录密码
		NickName string         `db:"nick_name"` // 用户昵称
		RoleName sql.NullString `db:"role_name"` // 角色名;管理员,普通用户
...
func (m *defaultAuthUsersModel) Insert(ctx context.Context, data *AuthUsers) (sql.Result, error) {
	...
	ret, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Username, data.Password, data.NickName, data.RoleName, data.RoleType, data.Email, data.Mobile)
	}, authUsersUserIdKey)
	return ret, err
}
...
func (m *defaultAuthUsersModel) Update(ctx context.Context, data *AuthUsers) error {
	...
	_, err := m.ExecCtx(ctx, func(ctx context.Context, conn sqlx.SqlConn) (result sql.Result, err error) {
		...
		return conn.ExecCtx(ctx, query, data.Username, data.Password, data.NickName, data.RoleName, data.RoleType, data.Email, data.Mobile, data.UserId)
	}, authUsersUserIdKey)
	return err
}
